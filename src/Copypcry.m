 ...
 %     float TIME_STEP,
  ...
  %------------------------------------------------------------------------------------------------------------------
    ...
    wakeID(i)	 = -1;
		...
		for(int j = 0; j < gridDim.x; j++)
            ...
            for(int yourSharedId = 0; yourSharedId < blockDim.x; yourSharedId++)
                ...
                if(i ~= yourId && yourId < NUM_PARTICLES)
                    ...
                    accX_i = accX_i + 0; % acc * (dx/r_soft);
                    accY_i = accY_i + 0; % acc * (dy/r_soft);
                    accZ_i = accZ_i + 0; % acc * (dz/r_soft);
                    ...
                    if(dz < 0.0) % If dz is negative you are below me.
                        if(r < r_min)
                            r_min	= r;
                            z_min	= dz;
                    ...
                    acc		= acc * (1.0 + r/ELECTRON_DEBYE)*exp(-r/ELECTRON_DEBYE)/mass_i;
			              acc = 0;	
                    accX_i 	= accX_i + (acc * (dx/(r_soft*r_soft*r_soft)));
                    accY_i 	= accY_i + (acc * (dy/(r_soft*r_soft*r_soft)));
                    accZ_i 	= accZ_i + (acc * (dz/(r_soft*r_soft*r_soft)));
                end
            end
        end
		...
		accZ_i = accZ_i + (charge1 * Ez / mass1);
...
__global__ void pcryCalculate_POS(
	...
	if(i < NUM_PARTICLES)
		if(TIME == 0.0)
			dustVelX(i) = dustVelX(i) + (0.5*DT*dustAccX(i));
			dustVelY(i) = dustVelY(i) + (0.5*DT*dustAccY(i));
			dustVelZ(i) = dustVelZ(i) + (0.5*DT*dustAccZ(i));
		else 
			dustVelX(i) = dustVelX(i) + (DT*dustAccX(i));
			dustVelY(i) = dustVelY(i) + (DT*dustAccY(i));
			dustVelZ(i) = dustVelZ(i) + (DT*dustAccZ(i));
        end

		dustPosX(i) = dustPosX(i) + (dustVelX(i)*DT);
		dustPosY(i) = dustPosY(i) + (dustVelY(i)*DT);
		dustPosZ(i) = dustPosZ(i) + (dustVelZ(i)*DT);
    end				
end
