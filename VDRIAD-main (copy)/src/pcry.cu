...
__global__ void pcryCalculate_ACC(
  ...
  //------------------------------------------------------------------------------------------------------------------
    ...
    BOLTZMANN = 1.380e-23;
    ION_MASS = 6.680e-26;
    BETACONSTANT = 1.44* 4.0 /3.0 *  GAS_PRESSURE  * sqrt(8.0 * pi * ION_MASS/BOLTZMANN/GAS_TEMP);
    SIGMACONSTANT = 2.0 * BOLTZMANN * GAS_TEMP / TIME_STEP;
    ...
    wakeDistanceR[i] = 10000.0;
    wakeDistanceZ[i] = 10000.0;
    wakeID[i]	 = -1;
		...
		for(int j = 0; j < gridDim.x; j++){
			...
			charge_j	= dustCharge[j];
			wakeCharge_j = wakeCharge[j];
			wakeLength_j = wakeLength[j];
            ...
            for(int yourSharedId = 0; yourSharedId < blockDim.x; yourSharedId++){
                ...
                if(i != yourId && yourId < NUM_PARTICLES){
                    ...
                    acc = -COULOMB * charge_j * charge_i * (1.0 + r / ION_DEBYE) * exp(-r / ION_DEBYE) / (r_soft * r_soft);
                    acc /= (mass_i);
                    ...
                    dz = (posZ_j - wakeLength_j) - posZ_i;

                    r_squared  	= dx*dx + dy*dy + dz*dz;
                    r_soft	= sqrt(r_squared + (epsilon * epsilon));
                    r  	   	= sqrt(r_squared);

                    acc 	= (COULOMB*charge_j*wakeCharge_j*charge_i);
                    acc		*=(1.0 + r/ELECTRON_DEBYE)*exp(-r/ELECTRON_DEBYE)/mass_i;
			              // acc = 0;	
		...
		// DRAG FORCE
        BETA = BETACONSTANT * (radius1*radius1)/mass1;
        ...
        SIGMA = sqrt(SIGMACONSTANT * BETA / mass1);
...
__global__ void pcryCalculate_POS(
	...
	for (int i = thread_id; i < NUM_PARTICLES; i += blockDim.x * gridDim.x) {
		if(TIME == 0.0f){
